...
void mm_to_theta_no_gate(dtbloc_t &dst_blocs, const dmbloc_t lhs_blocs,
                         const dmbloc_t rhs_blocs, bool conserve_left_right = false) {
...
void theta_to_mm(
  ...
  auto middle_indices = potential_middle_indices(theta_indices, direction_right);

  auto out_nondeg_deg = degeneracy_in_theta(theta_indices,middle_indices,direction_right);
